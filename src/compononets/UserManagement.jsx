...
function Categories() {
  ...
  const [email, setEmail] = useState("");  // State for new category input

  const [newCategoryDesc, setnewCategoryDesc] = useState("");  // State for new category input
  
  const [isLoading, setIsLoading] = useState(false);
  const [errorMessage, setErrorMessage] = useState("");
  const API = "http://127.0.0.1:8000/'admin-register/"; // API endpoint for categories
  ...
  const handleAdd = async () => {
    ...
    try {
      const res = await axios.post(
        API,
        { username: newCategoryInput , email:newCategoryDesc,email:email,},  // Send new category data in the body
        {
          headers: {
          ...
          <>
            ...
            {data.map((item) => (
              <div key={item.id} className="border p-4 rounded-md">
                ...
                <h2 className="font-bold">{item.password}</h2>
                <h2 className="font-bold">{item.mobile}</h2>
